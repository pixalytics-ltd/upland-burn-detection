...
def plot_line(xdata, ydata, stdata, product, outfile):
    ...
    ax1.xaxis.set_major_locator(ticker.MultipleLocator(15)) # Space out labels - every 5th
...
def plot_lines(xdata, ydata, stdata, xdata2, ydata2, stdata2, fnames, pnames, product2, outfile, graphics = True):
    ...
    spacing = int(len(xdata)/10)
    if len(xdata) < 5 or spacing == 0:
        spacing = 1
    #print("Xaxis spacing {}".format(spacing))
            
    # Setup colour palette
    cpal=['olive','green','orange','purple','red','blue']
    ...
    for pnum in range(prows):  # Each layer has its own plot
        ...
        for fnum in range(frows):
            pname = fnames[fnum]
            #print(pnum,fnum,ydata.shape,stdata.shape)
            ax[pnum].errorbar(x=xdata, y=ydata[fnum,pnum,:], yerr=stdata[fnum,pnum,:], linestyle='-', marker='None', color=cpal[fnum], label=pname)
            patches.append(mpatches.Patch(color=cpal[fnum], label=pname))

        # Setup axes formatting
        ax[pnum].xaxis.set_major_locator(ticker.MultipleLocator(spacing)) # Space out labels - every 10th
